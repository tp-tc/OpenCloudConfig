...
Configuration MaintenanceConfig {
  ...
  Script SendLogs {
    ...
    SetScript = {
      ...
      $body = Get-Content -Path (-join @(Get-ChildItem -Path ('{0}\log' -f $env:SystemDrive) | Where-Object { !$_.PSIsContainer -and $_.Name.EndsWith('.userdata-run.log') } | Sort-Object LastAccessTime))
