function Run-RemoteDesiredStateConfig {
  ...
  (New-Object Net.WebClient).DownloadFile(('{0}?{1}'$url, [Guid]::NewGuid()), $target)
...
if (-not ([Net.Dns]::GetHostName() -ieq (New-Object Net.WebClient).DownloadString('http://169.254.169.254/latest/meta-data/instance-id'))) {
