...
Configuration MaintenanceToolChainConfig {
  ...
  Script GpgKeyGenerate {
    ...
    SetScript = {
      ("Key-Type: 1`nKey-Length: 4096`nSubkey-Type: 1`nSubkey-Length: 4096`nName-Real: {0}`nName-Email: {0}@{1}.{2}`nExpire-Date: 0" -f $env:USERNAME.TrimEnd('$').ToLower(), $env:COMPUTERNAME.ToLower(), $env:USERDOMAIN.ToLower()) | Out-File ('{0}\Temp\gpg-gen-key.options' -f $env:SystemRoot)
