...
Configuration MaintenanceToolChainConfig {
  ...
  Script GpgKeyGenerate {
    ...
    GetScript = { @{ Result = ((Start-Process ('{0}\GNU\GnuPG\pub\gpg.exe' -f ${env:ProgramFiles(x86)}) -ArgumentList @('-u', 'Administrator', '--list-keys', ('{0}@{1}.{2}' -f $env:USERNAME.TrimEnd('$').ToLower(), $env:COMPUTERNAME.ToLower(), $env:USERDOMAIN.ToLower())) -Wait -NoNewWindow -PassThru -RedirectStandardOutput ('{0}\log\{1}.gpg-list-keys.stdout.log' -f $env:SystemDrive, [DateTime]::Now.ToString("yyyyMMddHHmmss")) -RedirectStandardError ('{0}\log\{1}.gpg-list-keys.stderr.log' -f $env:SystemDrive, [DateTime]::Now.ToString("yyyyMMddHHmmss"))).ExitCode -eq 0) } }
    SetScript = {
      ("Key-Type: 1`nKey-Length: 4096`nSubkey-Type: 1`nSubkey-Length: 4096`nName-Real: {0}`nName-Email: {0}@{1}.{2}`nExpire-Date: 0" -f $env:USERNAME.TrimEnd('$').ToLower(), $env:COMPUTERNAME.ToLower(), $env:USERDOMAIN.ToLower()) | Out-File -filePath ('{0}\Temp\gpg-gen-key.options' -f $env:SystemRoot) -encoding 'UTF8'
      ...
      Start-Process ('{0}\GNU\GnuPG\pub\gpg.exe' -f ${env:ProgramFiles(x86)}) -ArgumentList @('-u', 'Administrator', '--batch', '--gen-key', ('{0}\Temp\gpg-gen-key.options' -f $env:SystemRoot)) -Wait -NoNewWindow -PassThru -RedirectStandardOutput ('{0}\log\{1}.gpg-gen-key.stdout.log' -f $env:SystemDrive, [DateTime]::Now.ToString("yyyyMMddHHmmss")) -RedirectStandardError ('{0}\log\{1}.gpg-gen-key.stderr.log' -f $env:SystemDrive, [DateTime]::Now.ToString("yyyyMMddHHmmss"))
    }
    TestScript = { if ((Start-Process ('{0}\GNU\GnuPG\pub\gpg.exe' -f ${env:ProgramFiles(x86)}) -ArgumentList @('-u', 'Administrator', '--list-keys', ('{0}@{1}.{2}' -f $env:USERNAME.TrimEnd('$').ToLower(), $env:COMPUTERNAME.ToLower(), $env:USERDOMAIN.ToLower())) -Wait -NoNewWindow -PassThru -RedirectStandardOutput ('{0}\log\{1}.gpg-list-keys.stdout.log' -f $env:SystemDrive, [DateTime]::Now.ToString("yyyyMMddHHmmss")) -RedirectStandardError ('{0}\log\{1}.gpg-list-keys.stderr.log' -f $env:SystemDrive, [DateTime]::Now.ToString("yyyyMMddHHmmss"))).ExitCode -eq 0)  { $true } else { $false } }
  }
