...
curl http://taskcluster/aws-provisioner/v1/worker-type/${tc_worker_type} | jq -c 'del(.workerType, .lastModified) | (.regions[] | select(.region == "${aws_region}") | .launchSpec.ImageId) = "${aws_ami_id}"' | curl -H 'Content-Type: application/json' -X POST -d @- http://taskcluster/aws-provisioner/v1/worker-type/${tc_worker_type}/update
...
for region in "${aws_copy_regions[@]}"; do
  ...
  curl http://taskcluster/aws-provisioner/v1/worker-type/${tc_worker_type} | jq -c 'del(.workerType, .lastModified) | (.regions[] | select(.region == "${region}") | .launchSpec.ImageId) = "${aws_copied_ami_id}"' | curl -H 'Content-Type: application/json' -X POST -d @- http://taskcluster/aws-provisioner/v1/worker-type/${tc_worker_type}/update
done
